...
const tbl_Service = require("../../services/bus_trip_full_day_package_log");
var tbl_Model = require("../../models/bus_trip_full_day_package_log");
var langTitle = "";
var recievedData = ""
...
exports.Insert_New_Package =  async(req, res) => {
    ...
    try {
        ...
        let check_Ar_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
            var returnedList = await fun_check_existancy.check_title_existancy("bus_trip_full_day_package_log" , "" , "insert" , "ar" , val_Package_Title_Ar , "" , "" );
            resolve(returnedList);
        });

        let check_En_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
            var returnedList = await fun_check_existancy.check_title_existancy("bus_trip_full_day_package_log" , "" , "insert" , "en" , val_Package_Title_En , "" , "" );
            resolve(returnedList);
        });
        ...
        Promise.all([check_Ar_Title_Existancy_Promise , check_En_Title_Existancy_Promise]).then((results) => {
            ...
            if (results[0]==true && results[1]==true) {
                ...
                //#region insert process
                    new Promise(async (resolve, reject)=>{
                        const newList = await fun_insert_row.insert_row("bus_trip_full_day_package_log",sentData);
                        resolve(newList);
                    }).then((inserted_Flag) => {
                        if (!inserted_Flag) {
...
exports.update_Row =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            var exist = await fun_check_Existancy_By_ID.check_Existancy_By_ID("bus_trip_full_day_package_log",val_ID);
            resolve(exist);
        }).then((id_Exist) => {
            if (!id_Exist) {
                ...
                let check_Ar_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
                    var returnedList = await fun_check_existancy.check_title_existancy("bus_trip_full_day_package_log" , val_ID , "update" , "ar" , val_Package_Title_Ar , "" , "" );
                    resolve(returnedList);
                });

                let check_En_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
                    var returnedList = await fun_check_existancy.check_title_existancy("bus_trip_full_day_package_log" , val_ID , "update" , "en" , val_Package_Title_En , "" , "" );
                    resolve(returnedList);
                });
                ...
                Promise.all([check_Ar_Title_Existancy_Promise , check_En_Title_Existancy_Promise]).then((results) => {
                    ...
                    if (results[0]==true && results[1]==true) {
                        ...
                        new Promise(async (resolve, reject)=>{
                            const newList = await fun_update_row.update_row(val_ID, "bus_trip_full_day_package_log", recievedData , false);
                            resolve(newList);
                        }).then((update_flg) => {
                            if (!update_flg) {
...
exports.update_Suspend_Status_One_Row =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            let returnedList = await fun_check_Existancy_By_ID.check_Existancy_By_ID("bus_trip_full_day_package_log",val_ID);
            resolve(returnedList);
        }).then((returned_ID) => {
            if (!returned_ID) {
                //#region ID is not exist in DB msg 14
...
exports.update_Suspend_Status_Many_Rows =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            let returnedList = await fun_check_Existancy_By_List_IDS.check_Existancy_By_List_IDS("bus_trip_full_day_package_log",req.body.data);
            resolve(returnedList);
        }).then((returned_ID) => {
            if (!returned_ID) {
                //#region ID is not exist in DB msg 14
                ...
                if ( (status!="activate") && (status!="suspend") ){
                    ...
                    //#region update process
                    new Promise(async (resolve, reject)=>{
                        var exist = await fun_Update_Suspend_Status_Many_Rows.Update_Suspend_Status_Many_Rows(status , req.body.data , val_Updated_By , "bus_trip_full_day_package_log");
                        resolve(exist);
