...
exports.insert_promo_code_data =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            const newList = await tbl_Service.check_Existancy("insert" , "" , val_Code  , val_Title_En , val_Title_Ar , val_Percentage_Rate);
            resolve(newList);
        }).then((Exist_Flg) => {
            if (Exist_Flg.length>0) {
                //#region row already exist
                if (!Exist_Flg[0].Is_Suspended) {console.log("row already exist and active")
