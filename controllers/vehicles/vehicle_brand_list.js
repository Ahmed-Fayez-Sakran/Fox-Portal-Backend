...
const tbl_Service = require("../../services/vehicle_brand_list");
var tbl_Model = require("../../models/brand_name_lkp");
var langTitle = "";
var recievedData = ""
    ...
    try {
        ...
        let get_Message_Promise = new Promise(async (resolve, reject)=>{
            ...
            if (suspendStatus=="only-true"){
                result = await fun_handled_messages.get_handled_message(langTitle,123);
            }else if(suspendStatus=="only-false"){
                result = await fun_handled_messages.get_handled_message(langTitle,124);
            }else if(suspendStatus=="all"){
                result = await fun_handled_messages.get_handled_message(langTitle,125);
            }else{
...
exports.Insert_New_Brand =  async(req, res) => {

    try {
        //#region Global Variables
        langTitle = req.params.langTitle;
        var val_Serial_Number = req.body.Serial_Number;
        var val_Brand_Name_En = req.body.Brand_Name_En;
        var val_Brand_Name_Ar = req.body.Brand_Name_Ar;
        ...
        let check_Ar_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
            var returnedList = await fun_check_existancy.check_title_existancy("brand_name_lkp" , "" , "insert" , "ar" , val_Brand_Name_Ar , "" , "" );
            resolve(returnedList);
        });

        let check_En_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
            var returnedList = await fun_check_existancy.check_title_existancy("brand_name_lkp" , "" , "insert" , "en" , val_Brand_Name_En , "" , "" );
            resolve(returnedList);
        });
        ...
        Promise.all([check_Ar_Title_Existancy_Promise , check_En_Title_Existancy_Promise]).then((results) => {
            ...
            if (results[0]==true && results[1]==true) {
                ...
                new Promise(async (resolve, reject)=>{
                    var msg = await fun_handled_messages.get_handled_message(langTitle,150);
                    resolve(msg)
                }).then((msg) => {
                    msg = msg.replace("Brand_Name_Ar", val_Brand_Name_Ar);
                    msg = msg.replace("Brand_Name_En", val_Brand_Name_En);
                ...
                new Promise(async (resolve, reject)=>{
                    var msg = await fun_handled_messages.get_handled_message(langTitle,151);
                    resolve(msg)
                }).then((msg) => {
                    msg = msg.replace("Brand_Name_Ar", val_Brand_Name_Ar);
                    res.status(200).json({ data:[], message: msg, status: "already exist" });
                ...
                new Promise(async (resolve, reject)=>{
                    var msg = await fun_handled_messages.get_handled_message(langTitle,152);
                    resolve(msg)
                }).then((msg) => {
                    msg = msg.replace("Brand_Name_En", val_Brand_Name_En);
                    res.status(200).json({ data:[], message: msg, status: "already exist" });
                ...
                //#region insert process
                    new Promise(async (resolve, reject)=>{
                        const newList = await fun_insert_row.insert_row("brand_name_lkp",sentData);
                        resolve(newList);
                    }).then((inserted_Flag) => {
                        if (!inserted_Flag) {
                            ...
                            new Promise(async (resolve, reject)=>{
                                var result = await fun_handled_messages.get_handled_message(langTitle,153);
                                resolve(result);
                            }).then((msg) => {
                                if (langTitle=="en") {
                                    msg = msg.replace("Brand_Name_En", val_Brand_Name_En);
...
exports.update_Row =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            var exist = await fun_check_Existancy_By_ID.check_Existancy_By_ID("brand_name_lkp",val_ID);
            resolve(exist);
        }).then((id_Exist) => {
            if (!id_Exist) {
                ...
                let check_Ar_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
                    var returnedList = await fun_check_existancy.check_title_existancy("brand_name_lkp" , val_ID , "update" , "ar" , val_Brand_Name_Ar , "" , "" );
                    resolve(returnedList);
                });

                let check_En_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
                    var returnedList = await fun_check_existancy.check_title_existancy("brand_name_lkp" , val_ID , "update" , "en" , val_Brand_Name_En , "" , "" );
                    resolve(returnedList);
                });
                ...
                Promise.all([check_Ar_Title_Existancy_Promise , check_En_Title_Existancy_Promise]).then((results) => {
                    ...
                    if (results[0]==true && results[1]==true) {
                        ...
                        new Promise(async (resolve, reject)=>{
                            const newList = await fun_update_row.update_row(val_ID, "brand_name_lkp", recievedData , false);
                            resolve(newList);
                        }).then((update_flg) => {
                            if (!update_flg) {
                                ...
                                //#region msg update process successed
                                    new Promise(async (resolve, reject)=>{
                                        var result = await fun_handled_messages.get_handled_message(langTitle,154);
                                        resolve(result);
                                    }).then((msg) => {
                                        if (langTitle=="en") {
                                            msg = msg.replace("Brand_Name_En", val_Brand_Name_En);
...
exports.update_Suspend_Status_One_Row =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            let returnedList = await fun_check_Existancy_By_ID.check_Existancy_By_ID("brand_name_lkp",val_ID);
            resolve(returnedList);
        }).then((returned_ID) => {
            if (!returned_ID) {
                //#region ID is not exist in DB msg 14
                    ...
                    new Promise(async (resolve, reject)=>{
                        ...
                        if (!update_flg) {
                            ...
                            new Promise(async (resolve, reject)=>{
                                let result=""
                                if (status=="activate") {
                                    result = await fun_handled_messages.get_handled_message(langTitle,155);
                                } else {
                                    result = await fun_handled_messages.get_handled_message(langTitle,156);
                                }
                                resolve(result);
                            }).then((msg) => {
                                if (langTitle=="en") {
...
exports.update_Suspend_Status_Many_Rows =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            let returnedList = await fun_check_Existancy_By_List_IDS.check_Existancy_By_List_IDS("brand_name_lkp",req.body.data);
            resolve(returnedList);
        }).then((returned_ID) => {
            if (!returned_ID) {
                //#region ID is not exist in DB msg 14
                ...
                if ( (status!="activate") && (status!="suspend") ){
                    ...
                    //#region update process
                    new Promise(async (resolve, reject)=>{
                        var exist = await fun_Update_Suspend_Status_Many_Rows.Update_Suspend_Status_Many_Rows(status , req.body.data , val_Updated_By , "brand_name_lkp");
                        resolve(exist);
                        ...
                        if (!Records_Updated) {
                            ...
                            new Promise(async (resolve, reject)=>{                            
                                let result=""
                                if (status=="activate") {
                                    result = await fun_handled_messages.get_handled_message(langTitle,157);
                                } else {
                                    result = await fun_handled_messages.get_handled_message(langTitle,158);
                                }
                                resolve(result);
                            }).then((msg) => {
