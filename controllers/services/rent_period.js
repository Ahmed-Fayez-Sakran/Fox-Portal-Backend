...
const tbl_Service = require("../../services/rent_period");
...
exports.Insert_New_Package =  async(req, res) => {
    ...
    try {
        ...
        var val_Period_Ar = req.body.Period_Ar;
        var val_Period_En = req.body.Period_En;
        var val_Start_Range_Per_Day = req.body.Start_Range_Per_Day;
        var val_End_Range_Per_Day = req.body.End_Range_Per_Day;
        ...
        let check_Ar_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
            var returnedList = await fun_check_existancy.check_title_existancy("rent_period" , "" , "insert" , "ar" , val_Period_Ar , "" , "" );
            resolve(returnedList);
        });

        let check_En_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
            var returnedList = await fun_check_existancy.check_title_existancy("rent_period" , "" , "insert" , "en" , val_Period_En , "" , "" );
            resolve(returnedList);
        });
        ...
        Promise.all([check_Ar_Title_Existancy_Promise , check_En_Title_Existancy_Promise]).then((results) => {
            ...
            if (results[0]==true && results[1]==true) {
                ...
                new Promise(async (resolve, reject)=>{
                    var msg = await fun_handled_messages.get_handled_message(langTitle,99);
                    resolve(msg)
                }).then((msg) => {
                    msg = msg.replace("Period_Ar", val_Period_Ar);
                    msg = msg.replace("Period_En", val_Period_En);
                ...
                new Promise(async (resolve, reject)=>{
                    var msg = await fun_handled_messages.get_handled_message(langTitle,100);
                    resolve(msg)
                }).then((msg) => {
                    msg = msg.replace("Period_Ar", val_Period_Ar);
                    res.status(200).json({ data:[], message: msg, status: "already exist" });
                ...
                new Promise(async (resolve, reject)=>{
                    var msg = await fun_handled_messages.get_handled_message(langTitle,101);
                    resolve(msg)
                }).then((msg) => {
                    msg = msg.replace("Period_En", val_Period_En);
                    res.status(200).json({ data:[], message: msg, status: "already exist" });
                ...
                let sentData = new tbl_Model({
                    ...
                    Period_Ar: val_Period_Ar,
                    Period_En: val_Period_En,
                    Start_Range_Per_Day: val_Start_Range_Per_Day,
                    End_Range_Per_Day: val_End_Range_Per_Day,
                ...
                //#region insert process
                    new Promise(async (resolve, reject)=>{
                        const newList = await fun_insert_row.insert_row("rent_period",sentData);
                        resolve(newList);
                    }).then((inserted_Flag) => {
                        if (!inserted_Flag) {
                            ...
                            //#region insert process succeeded
                                new Promise(async (resolve, reject)=>{
                                    var result = await fun_handled_messages.get_handled_message(langTitle,102);
                                    resolve(result);
                                }).then((msg) => {
                                    if (langTitle=="en") {
                                        msg = msg.replace("Period_En", val_Period_En);
...
exports.update_Row =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            var exist = await fun_check_Existancy_By_ID.check_Existancy_By_ID("rent_period",val_ID);
            resolve(exist);
        }).then((id_Exist) => {
            if (!id_Exist) {
                ...
                let check_Ar_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
                    var returnedList = await fun_check_existancy.check_title_existancy("rent_period" , val_ID , "update" , "ar" , val_Period_Ar , "" , "" );
                    resolve(returnedList);
                });
        
                let check_En_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
                    var returnedList = await fun_check_existancy.check_title_existancy("rent_period" , val_ID , "update" , "en" , val_Period_En , "" , "" );
                    resolve(returnedList);
                });
                ...
                Promise.all([check_Ar_Title_Existancy_Promise , check_En_Title_Existancy_Promise]).then((results) => {
                    ...
                    if (results[0]==true && results[1]==true) {
                        ...
                        new Promise(async (resolve, reject)=>{
                            const newList = await fun_update_row.update_row(val_ID, "rent_period", recievedData , false);
                            resolve(newList);
                        }).then((update_flg) => {
                            if (!update_flg) {
                                ...
                                new Promise(async (resolve, reject)=>{
                                    var result = await fun_handled_messages.get_handled_message(langTitle,103);
                                    resolve(result);
                                }).then((msg) => {
                                    if (langTitle=="en") {
                                        msg = msg.replace("Period_En", val_Period_En);
...
exports.update_Suspend_Status_One_Row =  async(req, res) => {
    try {
        ...
        new Promise(async (resolve, reject)=>{
            let returnedList = await fun_check_Existancy_By_ID.check_Existancy_By_ID("rent_period",val_ID);
            resolve(returnedList);
        }).then((returned_ID) => {
            if (!returned_ID) {
                //#region ID is not exist in DB msg 14
                    ...
                    new Promise(async (resolve, reject)=>{
                        ...
                        if (!update_flg) {
                            ...
                            new Promise(async (resolve, reject)=>{
                                let result=""
                                if (status=="activate") {
                                    result = await fun_handled_messages.get_handled_message(langTitle,104);
                                } else {
                                    result = await fun_handled_messages.get_handled_message(langTitle,105);
                                }
                                resolve(result);
                            }).then((msg) => {
                                if (langTitle=="en") {
...
exports.update_Suspend_Status_Many_Rows =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            let returnedList = await fun_check_Existancy_By_List_IDS.check_Existancy_By_List_IDS("rent_period",req.body.data);
            resolve(returnedList);
        }).then((returned_ID) => {
            if (!returned_ID) {
                //#region ID is not exist in DB msg 14
                ...
                if ( (status!="activate") && (status!="suspend") ){
                    ...
                    //#region update process
                    new Promise(async (resolve, reject)=>{
                        var exist = await fun_Update_Suspend_Status_Many_Rows.Update_Suspend_Status_Many_Rows(status , req.body.data , req.body.Updated_By , "rent_period");
                        resolve(exist);
                        ...
                        if (!Records_Updated) {
                            ...
                            new Promise(async (resolve, reject)=>{                            
                                let result=""
                                if (status=="activate") {
                                    result = await fun_handled_messages.get_handled_message(langTitle,106);
                                } else {
                                    result = await fun_handled_messages.get_handled_message(langTitle,107);
                                }
                                resolve(result);
                            }).then((msg) => {
