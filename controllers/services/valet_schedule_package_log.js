...
const tbl_Service = require("../../services/valet_schedule_package_log");
var tbl_Model = require("../../models/valet_schedule_package_log");
var langTitle = "";
var recievedData = ""
    ...
    try {
        ...
        let get_Message_Promise = new Promise(async (resolve, reject)=>{
            ...
            if (suspendStatus=="only-true"){
                result = await fun_handled_messages.get_handled_message(langTitle,112);
            }else if(suspendStatus=="only-false"){
                result = await fun_handled_messages.get_handled_message(langTitle,113);
            }else if(suspendStatus=="all"){
                result = await fun_handled_messages.get_handled_message(langTitle,114);
            }else{
...
exports.Insert_New_Package =  async(req, res) => {
    ...
    try {
        ...
        var val_Package_Title_En = req.body.Package_Title_En;
        var val_Package_Title_Ar = req.body.Package_Title_Ar;

        var val_Package_Decription_En = req.body.Package_Decription_En;
        var val_Package_Decription_Ar = req.body.Package_Decription_Ar;        
        var val_Number_Of_Days = req.body.Number_Of_Days;
        var val_Number_Of_Hours = req.body.Number_Of_Hours;
        ...
        let check_Ar_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
            var returnedList = await fun_check_existancy.check_title_existancy("valet_schedule_package_log" , "" , "insert" , "ar" , val_Package_Title_Ar , "" , "" );
            resolve(returnedList);
        });

        let check_En_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
            var returnedList = await fun_check_existancy.check_title_existancy("valet_schedule_package_log" , "" , "insert" , "en" , val_Package_Title_En , "" , "" );
            resolve(returnedList);
        });
        ...
        Promise.all([check_Ar_Title_Existancy_Promise , check_En_Title_Existancy_Promise]).then((results) => {
            ...
            if (results[0]==true && results[1]==true) {
                ...
                new Promise(async (resolve, reject)=>{
                    ...
                    msg = msg.replace("Package_Title_Ar", val_Package_Title_Ar);
                    msg = msg.replace("Package_Title_En", val_Package_Title_En);
                ...
                //#region prepare Data Object for insertion
                 let sentData = new tbl_Model({
                    ...
                    Package_Title_En: val_Package_Title_En,
                    Package_Title_Ar: val_Package_Title_Ar,

                    Package_Decription_En: val_Package_Decription_En,
                    Package_Decription_Ar: val_Package_Decription_Ar,
                    Number_Of_Days:val_Number_Of_Days,
                    Number_Of_Hours:val_Number_Of_Hours,
                ...
                //#region insert process
                    new Promise(async (resolve, reject)=>{
                        const newList = await fun_insert_row.insert_row("valet_schedule_package_log",sentData);
                        resolve(newList);
                    }).then((inserted_Flag) => {
                        if (!inserted_Flag) {
...
exports.update_Row =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            var exist = await fun_check_Existancy_By_ID.check_Existancy_By_ID("valet_schedule_package_log",val_ID);
            resolve(exist);
        }).then((id_Exist) => {
            if (!id_Exist) {
                ...
                let check_Ar_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
                    var returnedList = await fun_check_existancy.check_title_existancy("valet_schedule_package_log" , val_ID , "update" , "ar" , val_Package_Title_Ar , "" , "" );
                    resolve(returnedList);
                });

                let check_En_Title_Existancy_Promise = new Promise(async (resolve, reject)=>{
                    var returnedList = await fun_check_existancy.check_title_existancy("valet_schedule_package_log" , val_ID , "update" , "en" , val_Package_Title_En , "" , "" );
                    resolve(returnedList);
                });
                ...
                Promise.all([check_Ar_Title_Existancy_Promise , check_En_Title_Existancy_Promise]).then((results) => {
                    ...
                    if (results[0]==true && results[1]==true) {
                        ...
                        new Promise(async (resolve, reject)=>{
                            const newList = await fun_update_row.update_row(val_ID, "valet_schedule_package_log", recievedData , false);
                            resolve(newList);
                        }).then((update_flg) => {
                            if (!update_flg) {
...
exports.update_Suspend_Status_One_Row =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            let returnedList = await fun_check_Existancy_By_ID.check_Existancy_By_ID("valet_schedule_package_log",val_ID);
            resolve(returnedList);
        }).then((returned_ID) => {
            if (!returned_ID) {
                //#region ID is not exist in DB msg 14
...
exports.update_Suspend_Status_Many_Rows =  async(req, res) => {
    ...
    try {
        ...
        new Promise(async (resolve, reject)=>{
            let returnedList = await fun_check_Existancy_By_List_IDS.check_Existancy_By_List_IDS("valet_schedule_package_log",req.body.data);
            resolve(returnedList);
        }).then((returned_ID) => {
            if (!returned_ID) {
                //#region ID is not exist in DB msg 14
                ...
                if ( (status!="activate") && (status!="suspend") ){
                    ...
                    //#region update process
                    new Promise(async (resolve, reject)=>{
                        var exist = await fun_Update_Suspend_Status_Many_Rows.Update_Suspend_Status_Many_Rows(status , req.body.data , val_Updated_By , "valet_schedule_package_log");
                        resolve(exist);
